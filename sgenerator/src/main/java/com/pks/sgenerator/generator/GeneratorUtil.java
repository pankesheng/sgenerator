...
public class GeneratorUtil {
	...
	static{
		ApplicationContext context = new FileSystemXmlApplicationContext(SGenUtil.class.getResource("").getPath()+"application.xml");
		if (context != null) {
	...
	private static void allFtl(Class<?> className) {
		...
		if (obj != null) {
			Map<String, Object> root = new HashMap<String, Object>();
			root.put("obj", obj);
			FreemarkerUtil.getInstance(freemarkerConfig).htmlFile(root,SGenUtil.class.getResource("").getPath()+"Page_xx_list.ftl",
					SAVEPATH + "/WEB-INF/ftl/admin/" + obj.getModuleName().toLowerCase() + "/" + obj.getClassName().toLowerCase()
							+ "_list.ftl");
			FreemarkerUtil.getInstance(freemarkerConfig).htmlFile(root,SGenUtil.class.getResource("").getPath()+"Page_xx_modify.ftl",
					SAVEPATH + "/WEB-INF/ftl/admin/" + obj.getModuleName().toLowerCase() + "/" + obj.getClassName().toLowerCase()
							+ "_modify.ftl");
		}
	}
